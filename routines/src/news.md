...
4. **Diversification.** Within each section, span geographic/linguistic sources. Among outlets (after rule 9's primary), rotate off the recurring outlet cluster — the preflight's Pressure section names it on its first line, computed from the rolling 30 days: when an equally authoritative registered outlet outside it carries the story, prefer it (for World, especially a non-European outlet; for Switzerland, one beyond SRF/Le Temps, DE/FR quota still met), so the outlet base widens across the week.
...
9. **Primary document rule.** When the story IS a document or an official act — a court ruling, a bill or law, a regulator's decision, an official report, a published legal opinion, a parliamentary or committee vote (the roll-call/result page or the body's press release), a government's stated position, recommendation or motion (the ministry's or Federal Council's own communiqué) — go find the primary itself (the court's, parliament's, regulator's, ministry's or party's own site) and cite it alongside the outlet, before settling for outlet-only sourcing. Search it by case number, bill number, docket or title; the outlet's article is what tells you the primary exists, not a substitute for it. If it is genuinely unreachable this run, say so in Gaps and name where it lives ("the judgment is on the tribunal's docket page, not yet published in full") instead of letting the press recap stand as the anchor.
