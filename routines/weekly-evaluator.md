...
- **Completeness first: every reasoned event gets a disposition.** Before any filtering, enumerate the window's reasoned feedback events — every `ev:"feedback"` event whose `reason` is a non-empty string (the fold writes `reason: ""` on bare votes; exclude only those), 👍 or 👎 — from the ledger, selected by the event's own `ts` falling in the window (never by the story's publish date; a vote cast this week on an older story still counts), so the list is exhaustive. Then give each one an explicit one-line disposition here: **applied** (name the proposal/patch id), **deferred** (name the threshold reason, e.g. "below the 2-distinct-stories bar"), or **rejected** (with the reason). No reasoned event may be silently omitted — the noise filter below decides what becomes a proposal, never what gets listed.
...
It ends `DONE`, or `FAILED (…)` — which needs a reaction: a commit failure means NOTHING was published (fix the reported error, rerun the same command); a push failure means the review is committed but not on origin (retry `git push origin HEAD:refs/heads/main` before the session ends). Never write the stub or run the git steps by hand.
