...
- Briefs: `_posts/{YYYY-MM-DD}-{slug}.md`; slugs `news`, `ai-ml`, `science`, `weekend`, `sports`,
  ...
  **Since 2026-07-18 the files are data-only**: `_config.yml` unpublishes all post pages (the
  homepage feed is the reading surface); only evaluator posts render a page, via a required
  `published: true` in their front matter. Do not delete `_posts/` files — homefeed/stats/evaluator
  read them.
- Site (2026-09-24): no theme. Own layouts `_layouts/{default,home,single,admin}.html`, one shared
  include `_includes/tokens.html` (palette, layer order, fonts, `window.__FB`), static
  `assets/css` (one `@layer` per file, zero `!important`) and `assets/js` (ES modules). The layout
  name `single` is pinned by `tools/publish.py`. Verify a site change locally before it reaches
  `main`: `tools/verify/build.sh` (GitHub's own Pages build in Docker) then `tools/verify/suite.mjs`
  (Playwright); see `tools/verify/README.md`. The page prints what the builder decides (`board`,
  `days`, `front`, periods) — put decisions in the builder, not in Liquid.
...
- Analytical plane (2026-07-18): `tools/plane/query.py` — SERVERLESS: the ledger is the database,
